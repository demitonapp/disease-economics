...
#: The finding/source/protection schemas are pinned from github.com/demitonapp/registers
#: (spec 09 P4) - vendor_registers.sh refreshes this, nothing here fetches it at runtime.
SCHEMA_DIR = "vendor/registers-research"
...
def _validators(root: Path):
    schemas = {p.name: json.loads(p.read_text()) for p in (root / SCHEMA_DIR).glob("*.schema.json")}
...
def validate(root: Path = ROOT):
    ...
    # ---- schema versions -------------------------------------------------
    for sp in sorted((root / SCHEMA_DIR).glob("*.schema.json")):
        ver = json.loads(sp.read_text()).get("x-schema-version", "")
        if not SEMVER.match(str(ver)):
            errors.append(f"{SCHEMA_DIR}/{sp.name}: x-schema-version must be MAJOR.MINOR.PATCH, got {ver!r}")
...
def check_against_base(root: Path, base: str):
    ...
    # Schema bump enforcement moved with the schemas themselves to demitonapp/registers'
    # own CI (scripts/bump_check.py, ported from the schema_bump_needed this used to run).
